...
sub create_db {
    ...
    ( $id, $rev ) =
      $self->db->put_doc( { dbname => 'zonemaster', doc => { doc_type => 'user', username => 'user1' } } );
...
# Find a recently started (within $minutes_between_tests_with_same_params) tests with the exact same params ($test_params_deterministic_hash)
    if ( $hashref ) {
        foreach my $test ( @$hashref ) {
            say Dumper( $test );
            if (   ( defined $test->{creation_time} )
                && ( $test->{creation_time} > time() - 60 * $minutes_between_tests_with_same_params ) )
            {
                $result = $test->{doc_id};
            }
        }
