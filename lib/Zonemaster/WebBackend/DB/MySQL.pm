...
sub user_authorized {
    ...
    my $href = $self->dbh->selectall_hashref(
        "SELECT id FROM users WHERE json_extract(user_info, 'username') = "
          . $self->dbh->quote( $user )
          . " AND json_extract(user_info, 'api_key') = "
...
sub create_new_test {
    ...
    my $query =
        "INSERT INTO test_results (batch_id, priority, params_deterministic_hash, params) SELECT * FROM (SELECT "
